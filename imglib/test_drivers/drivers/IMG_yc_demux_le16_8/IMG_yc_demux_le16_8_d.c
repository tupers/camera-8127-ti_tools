...
#include "IMG_yc_demux_le16_8_c.h"
...
main()
{
  ...
  for(count = 1; count <= testcases; count++)
  {
    ...
    memset(y_out_ref, 0, sizeof(y_out_ref[0]) * SIZE_Y);
    memset(cr_out, 0, sizeof(cr_out));
    memset(cr_out_ref, 0, sizeof(cr_out_ref[0]) * SIZE_C);
    memset(cb_out, 0, sizeof(cb_out));
    memset(cb_out_ref, 0, sizeof(cb_out_ref[0]) * SIZE_C);
