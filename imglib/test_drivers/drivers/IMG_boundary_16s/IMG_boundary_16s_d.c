...
main()
{
  ...
  for(count = 0; count < testcases; count++)
  {
    ...
    copy_int_to_short(src_ptr_1, inp, in_size);
    inp += in_size;
    memcpy(ref_coord_ptr, outp, out_size * sizeof(ref_coord[0]));
    outp += out_size;
    copy_int_to_short(ref_grey_ptr, outp, out_size);
    outp += out_size;
