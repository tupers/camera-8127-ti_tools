...
main()
{
  ...
  for(count = 1; count <= testcases; count++)
  {
    ...
    {
      printf("source array/mask size is less than test-case (ID %d : %d, %d)\n",
          count, in_size, k_size);
      return(-1);
    }
    ...
    copy_int_to_int(src_ptr_1, inp, in_size);   // copy input
    inp += in_size;
