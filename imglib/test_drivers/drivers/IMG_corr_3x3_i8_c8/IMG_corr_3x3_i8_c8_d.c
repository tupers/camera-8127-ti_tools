...
#include "IMG_corr_3x3_i8_c8_c.h"
...
main()
{
  ...
  for(count = 1; count <= testcases; count++)
  {
    ...
    copy_int_to_uchar(src_ptr_1, inp, in_size);   // copy input
    inp += in_size;
    copy_int_to_uchar(mask_ptr, inp, C_N);     // copy mask
    inp += C_N;
    ...
    /* run the testcase */
    t1 = clock();
    IMG_corr_3x3_i8_c8 (src_ptr_1, output_ptr, n_out, width, mask_ptr, shift, round);
    t2 = clock();
    ...
    t1 = clock();
    IMG_corr_3x3_i8_c8_cn (src_ptr_1, output_ptr, n_out, width, mask_ptr, shift, round);
    t2 = clock();
