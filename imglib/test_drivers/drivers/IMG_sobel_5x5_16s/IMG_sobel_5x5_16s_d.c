...
D_type input[SIZE];
D_type output[OUT_SIZE], ref_output[OUT_SIZE];
...
main()
{
  ...
  for(count = 1; count <= testcases; count++)
  {
    ...
    memset(output, 0, sizeof(output[0]) * OUT_SIZE);
    memset(ref_output, 0, sizeof(ref_output[0]) * OUT_SIZE);

    /* Copy input and ref output to respective buffers */
    copy_int_to_short(src_ptr, inp, in_size);   // copy input
    inp += in_size;
    memcpy(ref_output_ptr + OUT_OFFSET, outp, out_size * sizeof(outp[0]));
    outp += out_size;
    ...
    if( memcmp(output, ref_output, OUT_SIZE * sizeof(ref_output[0])) )
    {
