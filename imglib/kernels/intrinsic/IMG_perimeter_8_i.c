...
#pragma CODE_SECTION(IMG_perimeter_8,   ".text:intrinsic");
...
)
{
    ...
    _nassert((int)img_in  % 8 == 0);
    _nassert((int)img_out % 8 == 0);
    ...
    for ( i = 0; i < cols; i += 16)
    {
        top_dword0      = _amemd8_const(&img_in[i - cols]);
        mid_dword0      = _amemd8_const(&img_in[i       ]);
        bot_dword0      = _amemd8_const(&img_in[i + cols]);

        top_dword1      = _amemd8_const(&img_in[i - cols + 8]);
        mid_dword1      = _amemd8_const(&img_in[i        + 8]);
        bot_dword1      = _amemd8_const(&img_in[i + cols + 8]);
        ...
        _amem4(&img_out[i +  0]) = pixel0;
        _amem4(&img_out[i +  4]) = pixel1;
        _amem4(&img_out[i +  8]) = pixel2;
        _amem4(&img_out[i + 12]) = pixel3;
    }
...
/* ======================================================================== */
/*  End of file:  IMG_perimeter_8_i.c                                       */
/* ------------------------------------------------------------------------ */
/*            Copyright (c) 2008 Texas Instruments, Incorporated.           */
/*                           All Rights Reserved.                           */
/* ======================================================================== */
