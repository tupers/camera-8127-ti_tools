		...
		unsigned int max_hz, unsigned int spi_mode)
{
	...
	if (ret)
	{
		printf("spi flash read wrong id\n");
		goto err_read_id;
	}
	...
	case 0x01:
printf("spi flash probe spansion\n");	
		flash = spi_flash_probe_spansion(spi, idcode);
		break;
#endif
#ifdef CONFIG_SPI_FLASH_ATMEL
	case 0x1F:
	printf("spi flash probe atmel\n");
		flash = spi_flash_probe_atmel(spi, idcode);
		break;
#endif
#ifdef CONFIG_SPI_FLASH_MACRONIX
	case 0xc2:
	printf("spi flash probe macronix\n");
		flash = spi_flash_probe_macronix(spi, idcode);
		break;
#endif
#ifdef CONFIG_SPI_FLASH_WINBOND
	case 0xef:
	printf("spi flash probe winbond\n");
		flash = spi_flash_probe_winbond(spi, idcode);
		break;
#endif
#ifdef CONFIG_SPI_FLASH_MICRO
	case 0x20:
	printf("spi flash probe micro\n");
		flash = spi_flash_probe_micro(spi, idcode);
		break;
#endif
#ifdef CONFIG_SPI_FLASH_STMICRO
	case 0x20:
	printf("spi flash probe stmicro\n");
		flash = spi_flash_probe_stmicro(spi, idcode);
		break;
#endif
#ifdef CONFIG_SPI_FLASH_SST
	case 0xBF:
	printf("spi flash probe sst\n");
