...
static int omap2_mcspi_request_dma(struct spi_device *spi)
{
	...
	printk(KERN_INFO "omap2_mcspi_request_dma :chip_select[%d]\n",spi->chip_select);
	printk(KERN_INFO "omap2_mcspi_request_dma :rxdma[%d],txdma[%d]\n",mcspi_dma->dma_rx_sync_dev,mcspi_dma->dma_tx_sync_dev);
...
static int __init omap2_mcspi_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO"spi probe success\n");
	return status;

err4:
	printk(KERN_INFO"spi probe err4\n");
	kfree(mcspi->dma_channels);
err3:
	printk(KERN_INFO"spi probe err3\n");
	clk_put(mcspi->fck);
err2:
	printk(KERN_INFO"spi probe err2\n");
	clk_put(mcspi->ick);
err1a:
	printk(KERN_INFO"spi probe err1a\n");
	iounmap(mcspi->base);
err1aa:
	printk(KERN_INFO"spi probe err1aa\n");
	release_mem_region(r->start, (r->end - r->start) + 1);
err1:
	printk(KERN_INFO"spi probe err1\n");
