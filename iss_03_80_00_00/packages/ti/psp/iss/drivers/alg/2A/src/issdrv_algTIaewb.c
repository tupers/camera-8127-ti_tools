...
void *ALG_aewbCreate(int aewbNumWinH,int aewbNumWinV,int aewbNumPix)
{
  ...
  gTi2aControlParams.maxExposure         = 10000;
  ...
  gTi2aControlParams.dGainMax            = 8092;
#ifdef WDR_ON
  gTi2aControlParams.minExposure         = 200;//100;		//5000;		//100
  gTi2aControlParams.maxExposure         = 10000;//33333;
  gTi2aControlParams.dGainMin 		   = 1024;
  gTi2aControlParams.dGainMax 		   = 1024;
#else
  gTi2aControlParams.minExposure         = 200;		//5000;		//100
  gTi2aControlParams.maxExposure         = 10000;
#endif
...
void TI_Set_Exposure(unsigned int exposure)
{
  if(exposure > aeDynamicParams.exposureTimeRange[0].max)
  	exposure = aeDynamicParams.exposureTimeRange[0].max;
  if(exposure < aeDynamicParams.exposureTimeRange[0].min)
  	exposure = aeDynamicParams.exposureTimeRange[0].min;
  	
  ti2a_output_params.sensorExposure = exposure;
  ti2a_output_params.mask = 1;
}

int TI_Get_Exposure()
{
  return ti2a_output_params.sensorExposure;
}

int TI_Get_MaxExposure()
{
	return aeDynamicParams.exposureTimeRange[0].max;
}

int TI_Get_MinExposure()
{
  return aeDynamicParams.exposureTimeRange[0].min;
}

void TI_Set_Gain(unsigned int sensorGain,unsigned int pipegain)
{
	if(sensorGain > aeDynamicParams.sensorGainRange[1].max)
  		sensorGain = aeDynamicParams.sensorGainRange[1].max;
    if(sensorGain < aeDynamicParams.sensorGainRange[1].min)
  		sensorGain = aeDynamicParams.sensorGainRange[1].min;

	if(pipegain > aeDynamicParams.ipipeGainRange[2].max)
  		pipegain = aeDynamicParams.ipipeGainRange[2].max;
    if(pipegain < aeDynamicParams.ipipeGainRange[2].min)
  		pipegain = aeDynamicParams.ipipeGainRange[2].min;
  		  	
	ti2a_output_params.sensorGain = sensorGain;
	ti2a_output_params.ipipe_awb_gain.dGain = pipegain;

  	ti2a_output_params.mask = 6;
}

void TI_Get_Gain(int *sensorGain,int *pipegain)
{
  *sensorGain = ti2a_output_params.sensorGain;
  *pipegain = ti2a_output_params.ipipe_awb_gain.dGain;
}

void TI_Get_MaxGain(int *sensorGain,int *pipegain)
{
	*sensorGain = aeDynamicParams.sensorGainRange[1].max;
	*pipegain = aeDynamicParams.ipipeGainRange[2].max;
}

void TI_Get_MinGain(int *sensorGain,int *pipegain)
{
  	*sensorGain = aeDynamicParams.sensorGainRange[1].min;
  	*pipegain = aeDynamicParams.ipipeGainRange[2].min;
}
...
int TI_2A_config(int flicker_detection, int saldre)
{
  ...
  aeDynamicParams.thrld                       = gTi2aControlParams.thrld;
...
int ALG_aewbControl(void)
{
    ...
    if (gTi2aControlParams.update & TI2A_UPDATE_AE_DAY_NIGHT)
    {
      ...
      else
      {
        ...
        gTi2aControlParams.maxExposure         = 10000;//33330;
#endif
  ...
  if ( (gTi2aControlParams.update & TI2A_UPDATE_CONTROL_PARAMS_2A) || (new_awb_data_available == 1))
    ...
    {
    	generate_windows(WeightMatrixPrm,TI_WEIGHTING_MATRIX);
#if 0
...
int ALG_GetWeight(void *pWeightPrm,int len)
{
	if(len != sizeof(ALG_AewWeightParam))
		return -1;
		
	memcpy(pWeightPrm,&WeightMatrixPrm,len);
	return 0;
}

static int CheckWeightPrmValid(ALG_AewWeightParam *pPrm)
{
	if(pPrm->width1 > gALG_TI_aewbObj.AE_InArgs.statMat.winCtHorz)
		return 0;

	if(pPrm->height1 > gALG_TI_aewbObj.AE_InArgs.statMat.winCtVert)	
		return 0;

	if(pPrm->h_start2 +  pPrm->width2 > pPrm->width1)	
		return 0;

	if(pPrm->v_start2 +  pPrm->height2 > pPrm->height1)	
		return 0;		

	if((pPrm->weight < 0) || (pPrm->weight > 100)) 
		return 0;

	return 1;			
}
int ALG_SetWeight(void *pWeightPrm,int len)
{
	if(len != sizeof(ALG_AewWeightParam))
		return -1;

	if(!CheckWeightPrmValid((ALG_AewWeightParam *)pWeightPrm))
		return -1;
		
	memcpy(&WeightMatrixPrm,pWeightPrm,len);
